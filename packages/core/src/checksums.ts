...
const VAL36: Record<string, number> = Object.fromEntries(
  Array.from(ALPH36).map((ch, i) => [ch, i])
);

export const checksumFns: Record<string, ChecksumFn> = {
  luhn,
  mod10: luhn,
...
function modN(digits: string, n: number): boolean {
  let acc = 0;
...
export function mod10_3_1(digits: string): boolean {
...
export function mod3736(s: string): boolean {
  let p = 36;
...
export function s10CheckDigit(digits: string): boolean {
...
export function ontracCMod10(digits: string): boolean {
...
export function ontracDMod10(digits: string): boolean {
  if (!/^\d{14}$/.test(digits)) return false;
  const data = digits.slice(0, -1);
  const check = digits.charCodeAt(digits.length - 1) - 48;
...
function luhnOnDigits(data: string, check: number): boolean {
  let sum = 0,
    dbl = true;
